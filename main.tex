...
\newcommand{\brtack}{\hspace{-0.40em}\rotatebox[origin=c]{90}{\pmboxdrawuni{252F}}\hspace{0.15em}}
\newcommand{\seq}{\vdash}
...
\begin{longtabu}{>{$}c<{$}>{$}c<{$}}
    ...
    %\prftree[r]{\iruleL{\forall}}
        %{\Gamma,A[t/x] \seq \Delta}
        %{\Gamma,\forall x A \seq \Delta} &
    %\prftree[r]{\iruleR{\forall}}
        %{\Gamma \seq A[y/x],\Delta}
        %{\Gamma \seq \forall x A,\Delta} \\

    %\prftree[r]{\iruleL{\exists}}
        %{\Gamma,A[y/x] \seq \Delta}
        %{\Gamma,\exists x A \seq \Delta} &
    %\prftree[r]{\iruleR{\exists}}
        %{\Gamma \seq A[t/x],\Delta}
        %{\Gamma \seq \exists x A,\Delta}
\end{longtabu}

%Pour les règles des quantificateurs, on est soumis aux contraintes usuelles associées au système de termes : $t$ doit être librement substituable à $x$ dans $A$, et $y$ doit être librement substituable à $x$ dans $A$ en plus d'être non-libre dans $\Gamma$ et $\Delta$.
...
\begin{definition}
    On dit qu'une règle $\mathsf{R}$ est \emph{réversible} dans un système $\mathbf{D}$ lorsque pour toute instance $\mathsf{r}$ de $\mathsf{R}$ de prémisses $\sigma_1,\ldots,\sigma_n$ et de conclusion $\sigma$, si $\sigma$ est prouvable dans $\mathbf{D}$, alors $\sigma_i$ l'est aussi (pour $1 \leq i \leq n$).
\end{definition}
...
Afin de restreindre l'irréversibilité aux règles d'affaiblissement, il nous reste encore à trouver une version réversible des règles de la conjonction et de la disjonction. Toujours en suivant notre interprétation intuitive de la signification des séquents, on arrive aux règles suivantes, formulées pour la première fois pas Ketonen en 1944\footnote{Reprises ici du système $\mathbf{G3cp}$ introduit dans \cite{vPN01}.} :
