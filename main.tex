...
%Commençons par introduire le langage de $\mathbf{MALL}$, le fragment propositionnel et non-exponentiel de la logique linéaire \footnote{Les fragments exponentiel et du premier ordre ne nous seront d'aucune utilité pour les thèmes traités dans ce mémoire.} :

%\begin{definition}
	%Une \emph{formule} est définie inductivement comme étant soit :
	%\begin{itemize}
		%\item Un atome propositionnel $p$ ou son \emph{dual} $p^{\bot}$;
		%\item Une formule binaire $A * B$, où $A$ et $B$ sont des formules et $\ast$ est un connecteur parmi $\otimes,\oplus,\with,\parr$.
	%\end{itemize}
%\end{definition}
